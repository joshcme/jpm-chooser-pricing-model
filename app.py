...
# -------------------------------------------------------------------
# 6. Date selection and market snapshot
# -------------------------------------------------------------------
...
with cols[1]:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-label">Vanilla Call (T₂)</div>
...
with cols[2]:
    st.markdown(f"""
    <div class="metric-card">
        <div class="metric-label">Vanilla Put (T₂)</div>
...
with st.expander("🔍 Pricing Decomposition Details"):
    ...
    col1.metric("Call Leg (T₂)", f"${call_leg:.4f}")
    col2.metric("Put Leg (T₁, K')", f"${put_leg:.4f}")
    col3.metric("Total Chooser", f"${cf_price:.4f}")
    
    st.caption(f"Adjusted Strike K' = K × exp(-(r-q) × (T₂-T₁)) = {K * np.exp(-(r-q) * (T2-T1)):.2f}")

st.divider()

# -------------------------------------------------------------------
# 9. Monte Carlo Payoff Distribution
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 10. Sensitivity analysis with Plotly (FIXED - Removed subplot_kwargs)
# -------------------------------------------------------------------
...
# Create subplot with Plotly - Improved spacing (removed subplot_kwargs)
fig_sens = make_subplots(
    ...
    horizontal_spacing=0.15,  # Increased from 0.12 for more space
    vertical_spacing=0.18    # Increased from 0.15 for more space
)
...
sensitivities = [
    (grid_sigma, "sigma", "Volatility (σ)", 1, 1),
    (grid_K, "K", "Strike ($)", 1, 2),
...
# -------------------------------------------------------------------
# 11. Price Trend Visualization (FIXED - Better date display)
# -------------------------------------------------------------------
...
fig_trend.update_xaxes(
    type='category',  # Use category type for proper date display
    tickformat='%Y-%m-%d',
    tickangle=45,
    title_text="Date",
    row=2, col=1,
...
fig_trend.update_layout(
    ...
    hovermode='x unified',
    showlegend=False,
    margin=dict(l=50, r=50, t=60, b=80)  # Increased bottom margin for date labels
)
...
fig_trend.update_yaxes(
    ...
    tickfont=dict(size=10),
    range=[0, 0.8]  # Set reasonable range for volatility (0 to 80%)
)
...
with st.expander("📚 About This Dashboard"):
    ...
    **What is a Chooser Option?**
    A chooser option gives the holder the right to choose, at a predetermined date T₁, 
    whether the option will be a call or a put on the same underlying asset, with maturity T₂.
    
    **Pricing Methodology:**
    - **Closed-Form**: Based on the Rubinstein (1991) decomposition:
      `Chooser = Call(T₂) + Put(T₁, K')`, where `K' = K × exp(-(r-q)×(T₂-T₁))`
    - **Monte Carlo**: Simulates stock paths using geometric Brownian motion.
      The optimal decision at T₁ is to choose the call if `S(T₁) > K × exp(-(r-q)×(T₂-T₁))`,
    ...
    **Key Assumptions:**
    ...
    - European-style exercise at T₂
    
    **Interpretation:**
    - The chooser option price is always ≥ the vanilla call price at T₂
